...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!timing.ok) {
      console.warn('[leads] rejected on timing', { reason: timing.reason, elapsedMs: timing.elapsedMs });
    ...
    if (!quality.ok) {
      console.warn('[leads] rejected on email quality', { reason: quality.reason, domain: quality.domain });
    ...
    if (!supabaseKey) console.error('[leads] SUPABASE_SERVICE_ROLE_KEY is not set — the submission cannot be stored.');
